...
class Robot:
    ...
    def getLogs(self) -> str:
        ...
        while True:
            try:
                ...
                # with open(r'D:\205\ log\ all_err_log .md ','a+') as f:
                # f.write(data.decode())
                # print(data)
                logSock.close()
                return data.decode()
    ...
    def clearAlarm(self) -> bool:
        '''
        Clear the alarm of the robot.

        Returns:
            bool: True if the alarm is cleared, False otherwise.

        '''
        if self.connect_success:
            _, result = self.execute("clearAlarm")
            return result
        else:
            raise Exception(f"Robot not connected at IP:{self.ip_address}")

    def syncEncoders(self) -> bool:
        '''
        Sync the encoders of the robot.

        Returns:
            bool: True if the encoders are synced, False otherwise.

        '''
        if self.connect_success:
            _, result = self.execute("getMotorStatus")
            if result == 0:
                # Syncronize servo encoder data
                _, result = self.execute("syncMotorStatus")
                time.sleep(0.5)
                return result
            return False
        else:
    ...
    def getServoStatus(self) -> list:
        '''
        This function is used to get the servo status of the robot.
        ...
        if self.connect_success:
            ...
            if suc:
                ...
                raise Exception(f"Can not get servo status: {result}")
        else:
            raise Exception(f"Robot not connected at IP:{self.ip_address}")
    ...
    def setServoStatus(self, status: int) -> list:
        ...
        if self.connect_success:
            suc, result, id = self.sendCMD(
                "set_servo_status", {"status": status})
            time.sleep(1)
            if suc:
                return [id, result]
            else:
    ...
    # Movement Service
    # Joint Movement
    def moveByJoint(self, targetPos: list, speed: int, acc: int, dec: int, cond_type: int, cond_num: int, cond_value: int) -> list:
        '''
        This function is used to move the robot by joint.

        Attributes:
            targetPos (list): The target position in joint space.
            speed (int): The speed of the robot.
            acc (int): The acceleration of the robot.
            dec (int): The deceleration of the robot.
            cond_type (int): The condition type.
            cond_num (int): The condition number.
            cond_value (int): The condition value.

        Returns:
            list: [int, str] -> [ID, Result of the command]
        '''
        if self.connect_success:
            id, result = self.execute("moveByJoint", {
                "targetPos": targetPos,
                "speed": speed,
                "acc": acc,
                "dec": dec,
    ...
    def moveByJoints(self, pointList: list[list], speed: int = 30, acc: int = 10, dec: int = 10, cond_type: int = 0, cond_num: int = 7, cond_value: int = 1):
        '''
        This function is used to move the robot by joint.

        Attributes:
            pointList (list[list]): The list of target positions in joint space.
            speed (int): The speed of the robot.
            acc (int): The acceleration of the robot.
            dec (int): The deceleration of the robot.
            cond_type (int): The condition type.
        ...
        if self.connect_success:
            for i in range(len(pointList)):
                id, result = self.moveByJoint(
                    pointList[i], speed, acc, dec, cond_type, cond_num, cond_value)
                if not result:
                    raise Exception(f"Can not move by joint: {result}")
            return [id, result]
        else:
            raise Exception(f"Robot not connected at IP:{self.ip_address}")
